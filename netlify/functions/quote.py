...
def handler(event, context):
    ...
    try:
        ...
        if hist.empty:
            return {"statusCode": 404, "headers": headers, "body": json.dumps({"error": "no data"})}
        ...
        result = {
            ...
            "prev": safe(info.get("regularMarketPreviousClose") or info.get("previousClose") or (closes[-2] if len(closes)>1 else None)),
